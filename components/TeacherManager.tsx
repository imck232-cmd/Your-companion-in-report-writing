  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200">
        <h2 className="text-xl font-bold mb-4 text-teal-700">إضافة معلم جديد</h2>
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200">
        <h2 className="text-xl font-bold mb-4 text-teal-700">قائمة المعلمين</h2>
              ...
              <div key={teacher.id} className="p-4 border rounded-lg bg-slate-50">
                <div className="flex justify-between items-center">
                  {editingTeacher?.id === teacher.id ? (
                     <div className="flex gap-2 w-full">
                        ...
                        />
                        <button onClick={handleUpdateTeacher} className="bg-green-600 text-white px-3 rounded-md hover:bg-green-700">حفظ</button>
                        <button onClick={() => setEditingTeacher(null)} className="bg-slate-500 text-white px-3 rounded-md hover:bg-slate-600">إلغاء</button>
                    </div>
                  ) : (
                ...
                {teacherReports.length > 0 && (
                  <div className="mt-3 pt-3 border-t">
                    <h4 className="font-semibold text-sm mb-2 text-slate-600">التقارير المحفوظة:</h4>
                      ...
                      {teacherReports.map(report => (
                        <li key={report.id} className="flex justify-between items-center bg-white p-2 rounded-md border text-sm">
                          <div className="flex items-center gap-4">
                             <span className="font-semibold">{new Date(report.date).toLocaleDateString('ar-EG')}</span>
                             <span>النسبة النهائية: <span className="font-bold text-teal-600">{report.totalPercentage.toFixed(1)}%</span></span>
                          </div>
                          <div className="flex gap-2">
                            <button onClick={() => onEditReport(report)} className="text-blue-600 hover:underline">عرض/تعديل</button>
